...
// جلب البيانات مع التحقق أن رائد الأعمال هو صاحب المشروع
$stmt = $conn->prepare("
    SELECT 
        i.entrepreneur_signed, 
        i.type, 
        p.project_name, 
        i.amount, 
        i.equity_percentage,
    ...
    WHERE i.id = ? AND p.entrepreneur_id = ?
    LIMIT 1
");
$stmt->bind_param("ii", $investment_id, $entrepreneur_id);
$stmt->execute();
...
$entrepreneur_signed = intval($inv['entrepreneur_signed']);
...
// معالجة التوقيع مع PRG
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$entrepreneur_signed) {
    $update = $conn->prepare("UPDATE investments SET entrepreneur_signed=1 WHERE id = ? LIMIT 1");
    $update->bind_param("i", $investment_id);
    $update->execute();
    $update->close();
    $conn->close();
    header("Location: entrepreneur_sign.php?investment_id=" . $investment_id);
    exit;
}
$conn->close();
?>
  ...
  <div class="sign-container">
    ...
    <div class="agree-text" style="direction:rtl;">
      ...
      <?php else: ?>
        ...
        <b>البند الثالث: موضوع العقد</b><br>
        يلتزم المُقرض بتقديم مبلغ قدره <b><?= number_format($amount) ?> ريال سعودي</b>، ويُسدد المُقترض هذا القرض حسب الشروط المتفق عليها.<br><br>
        ...
        <ol>
          ...
          <li>التأخر يمنح الحق بالمطالبة بالمبلغ المتبقي.</li>
        </ol>
