...
// احصل على conv_id من GET أو POST
$conv_id = 0;
...
// إنشاء أو إعادة محادثة إذا جت project_id فقط
if (isset($_GET['project_id']) && !$conv_id) {
    $project_id = intval($_GET['project_id']);
    $stmt = $conn->prepare("SELECT investor_id FROM projects WHERE id = ? LIMIT 1");
    $stmt->bind_param("i", $project_id);
    $stmt->execute();
    $res = $stmt->get_result();
    if ($res->num_rows === 0) die("المشروع غير موجود");
    $investor_id = $res->fetch_assoc()['investor_id'];
    $stmt->close();
    ...
    $stmt->close();
    header("Location: entrepreneur_chat.php?conv_id=$conv_id");
    exit;
}
...
if (isset($_GET['fetch']) && $_GET['fetch'] == '1') {
    ...
    if ($msgs->num_rows === 0) {
        ...
        while ($m = $msgs->fetch_assoc()) {
            $cls = $m['sender_type'] === 'entrepreneur' ? 'entrepreneur' : 'investor';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['msg'])) {
    ...
    $stmt = $conn->prepare(
        "INSERT INTO messages
         ...
         VALUES (?, 'entrepreneur', ?, ?, NOW())"
    );
    $stmt->bind_param("iis", $conv_id, $entrepreneur_id, $msg);
    $stmt->execute();
    $stmt->close();
    exit;
}
...
$stmt->bind_param("ii", $conv_id, $entrepreneur_id);
$stmt->execute();
$res = $stmt->get_result();
if ($res->num_rows === 0) die("المحادثة غير موجودة أو غير مصرح لك بالدخول إليها.");
$row = $res->fetch_assoc();
$project_id    = $row['project_id'];
$project_name  = htmlspecialchars($row['project_name'], ENT_QUOTES);
$investor_name = htmlspecialchars($row['first_name'] . ' ' . $row['last_name'], ENT_QUOTES);
$stmt->close();
$conn->close();
?>
<html>
<head>
  <title>محادثة مع <?= $investor_name ?> – <?= $project_name ?></title>
  <style>
    body { font-family: 'Segoe UI', Tahoma, sans-serif; background: #f6f9f6; margin:0; }
    ...
    .chat-header { background:#0e6445; color:#fff; padding:14px 18px; display:flex; align-items:center; justify-content:space-between; }
    .chat-title { font-size:1.1em; font-weight:600; }
    .back-btn, .start-deal-btn { background:none; border:none; color:#fff; cursor:pointer; font-size:1.2em; }
    .start-deal-btn { background:#f3faff; color:#11775d; border-radius:6px; padding:6px 12px; font-size:.9em; font-weight:600; }
    ...
    .msg-bubble.entrepreneur { background:#11657e; color:#fff; text-align:right; }
    .msg-bubble.investor { background:#dbeff2; color:#205047; text-align:left; }
    .msg-time { display:block; font-size:0.65em; margin-top:4px; line-height:1.1; }
    .msg-bubble.entrepreneur .msg-time { color: rgba(255,255,255,0.85); }
    .msg-bubble.investor .msg-time { color:#555; }
...
<body>
  <div class="chat-container">
    <div class="chat-header">
      ...
      <span class="chat-title"><?= $investor_name ?> – <?= $project_name ?></span>
  ...
  <script>
    ...
    function loadMessages(){
      $.get('entrepreneur_chat.php', { conv_id: convId, fetch: 1 }, data => {
    ...
    $('#sendMsgForm').on('submit', function(e){
      ...
      $.post('entrepreneur_chat.php', { conv_id: convId, msg: msg }, res => {
        if (res === 'forbidden') {
